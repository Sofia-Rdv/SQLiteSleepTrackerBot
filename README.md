...
- `pip install pyTelegramBotAPI`   - Для установки библиотеки telebot
---

## Использование
После запуска бота доступны следующие шаги:
1. Отправьте команду `/start` для регистрации.
2. Используйте кнопку **'Сладких снов 😴'** или команду `/sleep`, чтобы зафиксировать время начала сна.
3. Нажмите кнопку **'Я проснулся ☀'** или команду `/wake`, чтобы завершить сессию сна.
4. Нажмите кнопку **'Качество сна 💫'** и выберите оценку качества (1-5) на появившейся клавиатуре.
5. Используйте кнопку **'Заметки 📝'** или команду `/notes`, чтобы добавить комментарий к оценке сна (например, 'Спалось нормально').
...
Настройка 
системы логирования осуществляется через следующие файлы:
...
- **Мокирование (unittest.mock):** 
  ...
  * **Тестирование отказоустойчивости:** Использование `patch` для имитации сбоев в работе `SQLite`. Это позволяет проверить корректность обработки исключений (блоки try...except) и надежность записи логов ошибок в файл `error.log`.
...
#### 1. Тест менеджера базы данных (Проверка добавления пользователя)
...
#### 2. Параметризованный тест функции-роутера бота (Проверка нажатия на inline кнопки.)
```python
import pytest
